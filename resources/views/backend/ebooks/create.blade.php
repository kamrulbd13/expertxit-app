    ...
    <div class="card">
        <div class="card-body">
            <h4 class="card-title text-center font-weight-bold text-base text-uppercase">
                {{ $isEdit ? 'Edit' : 'Add' }} Ebook
            </h4>
            <hr class="w-50 mx-auto">
            <form action="{{ $isEdit ? route('backend.ebooks.update', $ebook->id) : route('backend.ebooks.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if($isEdit)
                @method('PUT')
            @endif
            ...
            <!-- Title -->
                ...
                <div class="form-group mb-3">
                    ...
                    @if($isEdit && $ebook->cover_image)
                        <img src="{{ asset('storage/' . $ebook->cover_image) }}" width="80" class="mt-2">
                    @endif
                </div>
                ...
                <div class="text-center mt-3">
                    ...
                    <button type="submit" class="btn btn-success">
                        <i class="fa fa-check-circle"></i> {{ $isEdit ? 'Update' : 'Save' }}
                    </button>
                </div>
            </form>
        </div>
